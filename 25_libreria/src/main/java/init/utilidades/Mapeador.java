...
//*
	public Book jsonToBook(LibroJson libro) {
		int clasificacion=getClasificacion(libro.getPaginas());
			return new Book(libro.getTitulo(), libro.getTema().getNombreTema(), libro.getPrecio(), clasificacion);
	}
	
	private int getClasificacion(int paginas) {
		if (paginas>0 && paginas<=100)	return 1;
		else if (paginas>100 && paginas <=200) return 2;
		else return 3;
	}
//*/
}
